...
class Auth with ChangeNotifier{
  ...
  String get token {
    if (_expirtyDate != null && _expirtyDate.isAfter(DateTime.now()) && _token != null) {
      return _token;
    }
    return null;
  }
  ...
  Future<void> _authenticate(String email , String password,String type) async{
    final url = Uri.parse('https://identitytoolkit.googleapis.com/v1/accounts:$type?key=AIzaSyDPffeJ7MZX3oxu4Q4K1dpvfKLPxnmGvA0');
    try{
      ...
      _expirtyDate = DateTime.now().add(Duration(seconds: int.parse(responseData['expiresIn'])));
      _autoLogout();
      notifyListeners();
